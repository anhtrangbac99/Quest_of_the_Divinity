    ...
    for player_card in player_cards:
        for opponent_card in opponent_cards:
            ...
            if have_final_duel:
                ...
                reward_sacrified_final_duel.append(-100)
...
def Reward_Attack_Card(chosen_card,target_card):
    if chosen_card.card_id == -1 or target_card.card_id == -1 or chosen_card.status == 0:
        return -100
    ...
    reward_guard = chosen_card.isGuard()*10
...
def Reward_Attack_Avatar(chosen_card,opponent_life):
    if chosen_card.card_id == -1 or chosen_card.status == 0:
        return -100
    try:
...
def Reward_Buff(chosen_card,target_card):
    if chosen_card.card_id == -1 or target_card.card_id == -1:
        return -100
...
def Reward_Play(chosen_card,player_gold,target_card = None,player_life = 0,is_player = 0,is_empty=True):
    if is_player == 1 and chosen_card.card_id in [41,42,43]:
        return -100
    if is_player == 0 and chosen_card.card_id in [46,47]:
        return -100
    if chosen_card.gold > player_gold:
        return -100
    if chosen_card.card_id == -1:
        return -100
    reward_play = 0
    if target_card:
        if target_card.card_id == -1 or chosen_card.main_class not in [7,9]:
            return -100
        ...
        if chosen_card.card_id == 47:
            return target_card.cur_damage + target_card.cur_life + target_card.totalValue() + target_card.warchief + target_card.guard
