...
class DQNAgent(object):
    def __init__(self, gamma, epsilon, alpha, n_actions, replace_target_cnt,
        ...
        self.q_eval = DQNetwork(alpha=self.alpha,n_actions=self.n_actions,name=self.env_name+'_'+self.algo+'_q_eval',
                                input_dim=self.input_dims,chekpoint_dir=self.checkpoint_dir,device='cuda:1')
    ...
    def learn(self,writer=None):
        ...
        self.losses += loss
        if self.learn_step_count % BATCH_SIZE == 0:
            try:
                writer.add_scalar('training loss',self.losses/BATCH_SIZE,self.learn_step_count)
                self.losses = 0.0
            except:
                pass
            print(loss)
