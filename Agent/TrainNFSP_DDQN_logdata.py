...
if __name__ == '__main__':
    ...
    agentImitation = ImitationAgent(alpha=0.001,
                     ...
                     checkpoint_dir=NFSP_IMITATION_DIR_DDQN,algo='Imitation',
    ...
    agentDDQN = DDQNAgent(gamma=0.1, epsilon=0.7, alpha=0.0001,input_dim=INPUT_DIM,
                     ...
                     chkpt_dir=NFSP_DIR, algo='DDQNAgent',
                     env_name='Quest_of_Divinity')
    ...
    for i in range(NUM_EPOCHS):
        ...
        for idx,string_env in enumerate(dataset):
            ...
            if  dynamic < nuy:
                action,reward,state = agentDDQN.choose_action(string_env)
                ...
                agentDDQN.store(state,action,reward[action],next_state, False)
            ...
            if idx % 100 == 0:
                try:
                    smax = torch.nn.functional.softmax(torch.tensor([score_DDQN/num_DDQN,score_Imi/num_Imi]))
