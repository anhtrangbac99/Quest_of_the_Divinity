...
if __name__ == '__main__':
    ...
    agent_DQN = DQNAgent(gamma=0.9, epsilon=0.7, alpha=0.0001,replace_target_cnt=50,
                     input_dims=INPUT_DIM,
                     n_actions=NUM_ACTIONS, mem_size=50000, eps_min=0.1,
                     batch_size=32, replace=1000, eps_dec=1e-5,
    ...
    while True:
        if new_game:
            ...
            if user_id == 0:
                add = [(win[-1][0]+1,win[-1][1])]
            else:
                add = [(win[-1][0],win[-1][1]+1)]

            np.append(win, add,axis=0)
            np.save('fight',win)
            new_game = False
            data = conn.sock.recv(BUFFER_SIZE)
            string_env = json.loads(data.decode())
        ...
        if user_id == 1:
            agent_DQN.store(state,action,reward[action],next_state, False)
            agent_DQN.learn()
